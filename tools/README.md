...
| [`hr_rogue_lab.py`](hr_rogue_lab.py) | Builds deliberately rogue agent personas on the live platform and checks what HappyRobot's own audit system catches ([knowledge/14](../knowledge/14-rogue-agent-lab.md)). 13 personas cover all 6 danger families from [ar_fixtures.py](ar_fixtures.py); every tool webhooks into `AR_HOOK` — by default the permanent `/hook` on the deployed `angryrobot` service (Render), never a real system, so even the F2–F6 "dangerous" personas are safe to run live without anyone's laptop staying on. `demo` builds 5 agents with one random rogue category. Subcommands: `create`, `attack`, `report`, `corpus`, `all`, `demo` | `explore/rogue-lab/*` |
