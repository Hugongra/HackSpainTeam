...
| 16b | [Unsupervised clustering](16-unsupervised-clustering.md) | Can benign vs malignant be *learned* instead of hand-written? Clustering recovers **tool modality, not malignancy** (ARI +0.012, p=0.58); novelty scoring on surface features is worse than chance (AUC 0.354) while the engineered signals give 1.000; what clustering *is* good for | ✅ negative result, four feature spaces |
...
| 19 | [Activation probe](19-activation-probe.md) | A third detection ring: a **linear probe / representation-engineering direction** over a HF model's internal activations (mechanistic interpretability). Layer/token choice, the honest <1 ms probe-vs-forward latency split, the paired seed set, the OSS-models UI section, and caveats | ✅ mock demo + real-model path in `ar_probe.py` |
...
- 2026-09-19 — added 19 (activation probe): `ar_probe.py` — linear/repeng probe over HF model activations, OSS-models section in the pipeline UI; runs mock without torch.
