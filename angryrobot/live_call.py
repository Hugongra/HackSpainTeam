...
def persona_for(run_id: str, offered: list) -> dict:
    with _LOCK:
        ...
        if call is None:
            call = {"run_id": run_id, "started": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
                    "persona": draw(offered), "turns": 0, "verdicts": {}, "worst": "ALLOW", "killed": False, "alert": None,
                    "last": ""}
            _CALLS[run_id] = call
...
def calls(limit: int = 30) -> list:
    with _LOCK:
        ...
        return [{**c, "persona": {**c["persona"], "malicious_label": rounds.TRAITS[c["persona"]["malicious"]]["label"]
                                  if c["persona"]["malicious"] else None}} for c in out]
...
def build_router() -> APIRouter:
    ...
    def get_settings(x_angryrobot_secret: str | None = H, authorization: str | None = H):
        admin(x_angryrobot_secret, authorization)
        return settings()

    @router.post("/v1/live/settings")
    def set_settings(body: SettingsIn, x_angryrobot_secret: str | None = H, authorization: str | None = H):
        admin(x_angryrobot_secret, authorization)
    ...
    def list_calls(limit: int = 30, x_angryrobot_secret: str | None = H, authorization: str | None = H):
        admin(x_angryrobot_secret, authorization)
